...
Organization.prototype.addDepartment = async function () {
  ...
  const prompt = await inquirer
    ...
    .then((data) => {
      this.seedDatabase();
      return `Added ${data.department} to the database`;
    });
};
...
Organization.prototype.deleteDepartment = async function () {
  ...
  const prompt = await inquirer
    ...
    .then(async (deleted) => {
      ...
      this.seedDatabase();
      console.log(
        `Deleted the ${deleted.department} department from the database.`
      );
    })
    .then(async () => {
      this.mainMenu();
...
Organization.prototype.deleteRole = async function () {
  ...
  const prompt = await inquirer
    ...
    .then(async (deleted) => {
      ...
      this.seedDatabase();
      console.log(
        `Deleted the ${deleted.role} role from the database.`
      );
    })
    .then(async () => {
      this.mainMenu();
...
Organization.prototype.deleteEmployee = async function () {
  ...
  const prompt = await inquirer
    ...
    .then(async (deleted) => {
      ...
      this.seedDatabase();
      console.log(
        `Deleted ${deleted.employee} from the database.`
      );
    })
    .then(async () => {
      this.mainMenu(), 5000;
...
Organization.prototype.mainMenu = async function (msg) {
  ...
  if (msg) {
    console.log(gradient.passion(msg));
  }
